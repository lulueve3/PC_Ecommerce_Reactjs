  ...
  const fetchPosts = async (page = 0, keyword = "") => {
    try {
      ...
      const response = await axios.get(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/forums/questions`,
        {
          params: {
            page: page,
            size: 10,
      ...
      const formattedPosts = response.data.results.map((post) => ({
        ...post,
        commentCount: post.answers.length, // Add commentCount based on answers array length
      }));
