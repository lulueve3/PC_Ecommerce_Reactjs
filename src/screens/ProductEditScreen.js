...
const ProductEditScreen = () => {
  ...
  const submitHandler = (e) => {
    ...
    if (!isVariantPricingValid) {
      toast.error(
        "Compare-at price must be greater than the price for all variants."
      );
      return;
    }
  ...
  useEffect(() => {
    const getCollections = async () => {
      ...
      const { data } = await axios.get(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/collections?page=0&size=100&sortBy=id&sortDirection=ASC`,
        config
      );
      console.log(data);
  ...
  const handleVariantChange = (index, field, value) => {
    ...
    if (
      ...
      if (field === "compareAtPrice" && value >= 0)
        updatedVariants[index][field] = value;
      else updatedVariants[index][field] = 1;
    }
  ...
  const addNewImage = async (index, imageUrl) => {
    try {
      ...
      await axios.post(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/products/${productId}/images`,
        {
    ...
    try {
      ...
      for (let index = 0; index < selectedImages.length; index++) {
        ...
        const response = await axios.post(
          "https://api.cloudinary.com/v1_1/dommm7bzh/image/upload",
          data
        );
  ...
  const handleReplaceImage = async (e, imageId) => {
    ...
    try {
      ...
      await axios.patch(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/products/${productId}/images/${imageId}`,
        {
          src: newImageUrl,
  ...
  const handleRemoveImage = async (e, idToRemove) => {
    ...
    try {
      ...
      await axios.delete(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/products/${productId}/images/${idToRemove}`,
        config
      );
    } catch (error) {
  ...
  const updateCollection = async () => {
    try {
      ...
      await axios.patch(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/products/${productId}/collections`,
        {
          productId: 1,
          collectionIds: selectedCollections,
        },
                ...
                {images
                  ...
                  .map((image, index) => (
                    <div key={image.id}>
                      ...
                      />
                      {image.position !== 0 && (
                        <button onClick={(e) => handleRemoveImage(e, image.id)}>
                          Remove
                        </button>
                      )}{" "}
                    </div>
            ...
            <Card.Body>
              <div className="mb-3">
                ...
                <Select
                  ...
                  value={optionsCollection?.filter((option) =>
                    ...
                    <Row className="mb-3">
                      <Col xs={12} md={3}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleVariantChange(0, "price", e.target.value)
                          }
                      ...
                      <Col xs={12} md={3}>
                        ...
                        <input
                          ...
                          id={`variantPriceDefault`}
                          value={
                            (variants[0] && variants[0].compareAtPrice) || 0
                          }
                          onChange={(e) =>
                            handleVariantChange(
                              0,
                      ...
                      <Col xs={12} md={2}>
                        <label
                          htmlFor={`variantQuantityDefault`}
                          className="form-label"
                        >
                          Quantity
                        </label>
