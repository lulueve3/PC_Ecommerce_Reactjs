  ...
  const fetchQuestions = async () => {
    try {
      ...
      const response = await axios.get(
        "http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/forums/questions",
        {
          params: {
  ...
  const handleApproveQuestion = async (id, approved) => {
    try {
      ...
      const response = await axios.patch(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/admin/forums/questions/${id}`,
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {questions?.map((question) => (
            <tr
              key={question.id}
              style={{ backgroundColor: question.active ? "#BAD4AA" : "" }}
            >
              <td>{question.id}</td>
