    ...
    if (buildDetails && buildDetails.items) {
      buildDetails.items.forEach((item) => {
        ...
        for (let i = 0; i < item.quantity; i++) {
          // Add each item based on its quantity
          dispatch(
            addToCart({
  ...
  const fetchPcBuild = async () => {
    ...
    try {
      const response = await axios.get(
        `http://mousecomputer-api.southeastasia.cloudapp.azure.com/api/pc-builds/code/${buildCode}`
      );
          ...
          <Table striped bordered hover>
            <thead>
              <tr>
                ...
                <th>Quantity</th> {/* Thêm cột Quantity */}
                <th>Price</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <Link to={`/product/${item.part.productId}`}>
                      ...
                      {item.part.title}
                      {item.part.variantTitle !== "Default Title" &&
                      item.part.variantTitle
                        ? ` - ${item.part.variantTitle}`
                        : ""}
                    </Link>
                  </td>
              ...
              <tr>
                <td colSpan="4" style={{ textAlign: "center" }}>
                  <Button
                    ...
                    disabled={
                      !buildDetails ||
                      buildDetails.items.every(
                        (item) => item.part.quantity <= 0
                      )
                    }
                  >
                    Add All to Cart
